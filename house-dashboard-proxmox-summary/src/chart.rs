...
pub fn draw_proxmox_summary<S>(
    ...
    statuses: &HashMap<String, String, S>,
    ...
    S: BuildHasher,
{
    ...
    let title_height = draw_title(proxmox_summary.title.as_str(), style, &root)?;
...
fn prepare_sorted_loads<'a, 'b, S>(
    ...
    statuses: &'b HashMap<String, String, S>,
    ...
    S: BuildHasher,
{
    ...
    hosts
        .into_iter()
        .map(|host| {
            let load: Option<f64> = statuses
                .get(host)
                .into_iter()
                .filter_map(|status| {
                    if status == "running" {
                        loads.get(host).copied()
                    } else {
                        None
                    }
                })
